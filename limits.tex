...
\begin{definition}[Category of co-cones]
\label{def:category_of_cocones}
Let $F$ is a \mynameref{def:diagram_of_shape} $\cat{I}$ in $\cat{C}$.

TBD

The category of \mynameref{def:cocone}s is denoted as $\Delta
\downarrow F$ \cite{wiki:cone} 
\end{definition}
...
Let $F$ is a \mynameref{def:diagram_of_shape} $\cat{I}$ in $\cat{C}$
and $\Delta \uparrow F$ is the \mynameref{def:category_of_cocones}.

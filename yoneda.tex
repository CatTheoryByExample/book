...
In the chapter we shall assume that the category $\cat{C}$ is a
...
\mynameref{def:con_hom_functor}. But in the case we consider the set of
morphisms from an arbitrary object $x$ to the picked object $a$ i.e.
we reverse \mynameref{def:arrow}s in this case.
...
Let $\cat{C}$ be a \mynameref{def:localy_small_category} and $a \in
...
\catob{C}$. Consider \mynameref{def:functor} from $\cat{C}^{op}$ to
the \mynameref{def:setcategory}, equivalently a
\mynameref{def:contravariant_functor} from $\cat{C}$ to
\mynameref{def:setcategory}, defined by the following rules
\begin{itemize}
\item $\forall x \in \catob{C}$ define an object in the set category:
...
Consider the category $\cat{Set}$. \mynameref{def:morphism_set}
$\catmset[Set]{a}{x}$ is the same as the \mynameref{def:exponential}
object $x^a$, i.e.
\[
\catmset[Set]{a}{x} \cong x^a.
\]
More generally, the same notation makes sense in a category where the
exponential object $x^a$ is defined and interpreted as the object of
morphisms from $a$ to $x$. Therefore, in such context, formally, we can write
\[
...
$\catmset[C]{a}{y}$. Functor $F$ maps the 2 objects into $F(x)$ and

...
\item $\forall f: x \to y \in \cathom{C}$ define a function in the set category
  $Hom_c(a, f): Hom_c(a, x) \to Hom_c(a, y)$ as follows
  $Hom_c(a, f) = \{f \circ g | g \in Hom_c(a, x)\}$.
\end{itemize}  
...
\label{ex:cov_hom_functor}
  \begin{figure}
    ...
    \begin{tikzpicture}[ele/.style={fill=black,circle,minimum
      ...
      \node[ele,label=right:$d$] (d) at (1,1.5) {};
      ...
      \draw[->,thick,shorten <=2pt,shorten >=2pt] (b) to
           [out=45,in=135,looseness=20] node[above] {$\idm{b}$}
           (b); 

      \draw[->,thick,shorten <=2pt,shorten >=2pt] (c) to
           [out=-45,in=-135,looseness=20] node[below] {$\idm{c}$}
           (c); 
      ...
      \draw[->,thick,shorten <=2pt,shorten >=2pt] (a) to
           ...
           (c); 
      \draw[->,thick,shorten <=2pt,shorten >=2pt] (a) to
           [out=-45,in=45,looseness=0.5] node[right] {$f^{(c)}_2$} (c); 


      \node[draw,fit= (a) (b) (c) (d),minimum width=5.5cm, minimum
        height=5.5cm] {} ;

    \end{tikzpicture}
    \caption{Covariant Hom functor example. Category $\cat{C}$}

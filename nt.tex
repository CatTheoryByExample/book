...
The natural transformation is not an easy concept compared to other ones
...
\cref{fig:nt_objects_mapping}). There are 2 possible options:
\begin{enumerate}
...
\item $\exists \alpha_a \in \catmset{a_F}{a_G} \subset
  \cathom{D}$. 
\end{enumerate}
We can of course create an artificial morphism that connects the
...
considered functors are related to each other. As an opposite example, if there
...
\[
\alpha \star \beta: J \circ F \tont K \circ G
\] 
...
\mynameref{def:object}s of 3 categories: $\cat{\mathcal{A}} = \left[\cat{C},
  ...
  I.e.
  \[
  \otimes: F \times J \to J \circ F \in \catob{\mathcal{C}}.
  \]
...
\begin{equation}
\label{eq:lw}
\alpha H = \idnt{H} \star \alpha
\end{equation}

For right whiskering:
\begin{equation}
\label{eq:rw}
H \alpha = \alpha \star \idnt{H}
\end{equation}
\end{remark}

...
\cref{ch:monoid_categorically}  we will continue the 
...
universal construction (\mynameref{def:universalproperty}) for the
object definition. 
...
For our example consider 2 \mynameref{def:set}s $X, Y$ and there is a
function (morphism) $f: X \to Y$. Our claim was that the function $f$ can
be associated with another object (set)  $Y^X$. 

Why do we use so strange notation (exponential)? Lets $X$ has only 2
elements, i.e. its \mynameref{def:cardinality} is $2$: 
$\left|X\right|= 2$. 
The set $Y$ is \mynameref{def:singleton_set} i.e. its
\mynameref{def:cardinality} is $1$: $\left|Y\right| = 1$. 

Consider a function $f: X \to Y$. How many such functions do we have?
There are really 2 functions. One of them return the first element
from $Y$ and the other returns the second one. The number of functions
can be written as 
...
possible choice for it: just return the only possible element from
...
related to notion of currying that is defined as follows.
...
f:(X\times Y) \to Z
\]
\textit{currying} constructs a new function
\[
h : X \to (Y \to Z)
\]
...
\begin{remark}[Currying and Sets]

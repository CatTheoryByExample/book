...
The important question is how we can decide whether an
...
structure, but can use a \mynameref{def:categorical_approach} (see \cref{ch:categorical_approach}) i.e. if we cannot use a ``microscope'' let's
use a ``telescope'' and define the equality of objects and morphisms of
a category $\cat{C}$ in terms of the whole $\cathom{C}$.

\begin{definition}[Objects equality and canonical isomorphism]
\label{def:object_equality}
Two \mynameref{def:object}s $a$ and $b$ in \mynameref{def:category}
$\cat{C}$ are equal if they are the same element of the
\mynameref{def:class} $\catob{C}$.

They are isomorphic if there exists a \mynameref{def:isomorphism}
$a \cong_f b$. This also means that there exists an isomorphism
$b \cong_g a$. These two \mynameref{def:morphism}s ($f$ and $g$) are
related to each other via the following equations:
$g \circ f = \idm{a}$ and $f \circ g = \idm{b}$.

If there exists a unique isomorphism between $a$ and $b$, then $a$ and
$b$ are canonically isomorphic. In this case the objects can be treated
as the same object for categorical purposes, although they are not
necessarily equal as objects of $\cat{C}$.
\end{definition}
...
following definition of morphism equality:
...
\end{itemize}
or \mynameref{def:commutative_diagram}s that postulate the equality.
\end{definition}
...
\begin{proof}
Consider an \mynameref{def:object} $a$ and its \mynameref{def:id}
$\idm{a}$. Assume existence of a function $f: a \to a$ such that $f$ is also
identity. \eqref{eq:leftid}, for $f$ as identity, gives us
\[
f \circ \idm{a} = \idm{a}.
\]
On the other hand \eqref{eq:rightid} for $\idm{a}$ is satisfied:
\[
f \circ \idm{a} = f
\]
i.e.
...
Let $\cat{C}$ be a \mynameref{def:category}. The
...
Note that there is only one function from the empty set to any other set
\cite{bib:proofwiki:Empty_Mapping_is_Unique} that makes the empty set
the \mynameref{def:initial_object} in \mynameref{def:setcategory}.
\end{example}

\begin{theorem}[Initial object is unique]
\label{thm:initial_object_unique}
Let $\cat{C}$ be a category and let $i, i' \in \catob{C}$ be two
\mynameref{def:initial_object}s. Then there exists a unique
...
\cref{fig:initial_object_unique}). Since $i$ is an initial object,
$\exists! \, u: i \to i'$. On the other hand $i'$ is also an initial object
...
only one morphism $\idm{i}: i \to i$. The same is true for $i'$.
Therefore $u^{-1} \circ u = \idm{i}$ and $u \circ u^{-1} = \idm{i'}$.
This completes the commutative diagram and finishes the proof.
\begin{figure}
  \centering
...
\mynameref{def:morphism}s $\pi_1: c \to c_1, \pi_2: c \to c_2$ such
that the following universal property is satisfied: $\forall c' \in \catob{C}$
...
\mynameref{def:morphism}s $i_1: c_1 \to c, i_2: c_2 \to c$ such that
the following \mynameref{def:universalproperty} is satisfied: $\forall c'
\in \catob{C}$

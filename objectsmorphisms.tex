...
If there are 2 sets $X, Y$ and there is a morphism
$f: X \to Y$ then the object (set) $Y^X$ is identical to the morphism
$f$. Why do we use so strange 
notation (exponential)? Consider a function from a set with only one
element (\mynameref{def:terminal_object}) $X$ to a set with 2
elements $Y$. How many such functions do we have? There are really 2
functions. One of them return the first element from $Y$ and the other
returns the second one. The number of functions can be written as
$2^1$. I.e. one can write
\begin{equation}
\label{eq:exponential_size}
\left|Y^X\right| = \left|Y\right|^{\left|X\right|},
\end{equation}
where $\left|X\right| = 1$ - the number of elements in $X$, 
$\left|Y\right| = 2$ - the number of elements in $Y$.

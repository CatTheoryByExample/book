...
\mint{haskell}{Void} and I will provide you the result of
evaluation''.  

There is no function in opposite direction because it would had been
used for the \mint{haskell}{Void} object creation. 
\end{example}
...
\mint{haskell}{b -> a} and \mint{haskell}{b -> c}. I.e.
...
result cannot be created: \mint{c++}{void}.
...
\end{minted}
Really any \mint{c++}{struct} or \mint{c++}{class} can be
...
\begin{example}[Sum][\textbf{C++}]
  ...
  to keep one of two types is \mint{c++}{union}. 

  %% There is an \mint{haskell}{Either} implementation from
  ...
  C++17 suggests \mint{c++}{std:variant} as a safe replacement
  for \mint{c++}{union}. The example of the \mint{c++}{factor}
  function is below
  \begin{minted}{c++}
    template <typename A, typename B, typename C, typename D>

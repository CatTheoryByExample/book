...
\begin{definition}[Functor]
  \label{def:functor}
  Let $\cat{C}$ and $\cat{D}$ be 2 categories. A mapping $F: \cat{C}
...
\begin{remark}[Functor]
  When we say that a functor preserves internal structure we assume that
  the functor is not just mapping between \mynameref{def:object}s but
  also between \mynameref{def:morphism}s.  

  Thus a functor is something that allows us to map one category into another.
  The initial category can be considered as a pattern thus the mapping
  is some kind of search for the pattern inside another category.
\end{remark}
...
\begin{example}[Functor][$\cat{C\texttt{++}}$]
  ...
  and therefore can convert one type to another. For instance the
  list of strings can be obtained with the following construction:
...
\begin{definition}[Category Product]
  ...
  \item \mynameref{axm:composition} is defined as follows 
    ...
    \)
  \item Identity is defined objectwise as follows:
    $\idm{(c,d)} = \left(\idm{c}, \idm{d}\right)$
  \end{itemize}
\end{definition}

...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[ele/.style={fill=black,circle,minimum
    ...
    \draw[->,thick,shorten <=2pt,shorten >=2pt] (a) to
    node[pos=0.5,above]{$f$} (b); 
    \draw[->,thick,shorten <=2pt,shorten >=2pt] ([yshift=0.5] c.east) to         
         node[pos=0.5,above]{$g_1$} ([yshift=0.5] a.west); 
    \draw[->,thick,shorten <=2pt,shorten >=2pt] ([yshift=-0.5] c) to
         node[pos=0.5,below]{$g_2$} ([yshift=-0.5] a); 
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[ele/.style={fill=black,circle,minimum
    ...
    node[pos=0.5,above]{$f$} (b); 
    \draw[->,thick,shorten <=2pt,shorten >=2pt] (b) to
         [out=45,in=135,looseness=2]
         node[pos=0.5,above]{$g_1$} (c); 
    \draw[->,thick,shorten <=2pt,shorten >=2pt] (b) to
         [out=-45,in=-135,looseness=2]
         node[pos=0.5,below]{$g_2$} (c); 
...
\label{def:morphism_set}
  ...
  as $\catmset{a}{b}$ if the category does not matter 
\end{definition}
...
$\cathom{C}$ is a \nameref{def:set}. The set is called \textit{homset}.
\end{definition}
...
\begin{remark}[Injection vs Monomorphism]
  ...
  \mynameref{def:monomorphism} in $\cat{Set}$ category. Moreover
  there is a proof 
...
\begin{definition}[$\cat{C++}$ category]
...
\subsection{$\cat{C++}$ toy category}
\begin{example}[$\cat{C++}$ toy category]
  \label{ex:cppcategory}
  \index{Object!$\cat{C++}$ example}
  \index{Morphism!$\cat{C++}$ example}

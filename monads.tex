...
\begin{minted}{haskell}
    class Functor m => Monad m where
        ...
        join  :: m (m a) -> m a
\end{minted} 
where \mint{haskell}{return} can be treated as $\eta$
\eqref{eq:monad_eta} and 
\mint{haskell}{join} as $\mu$ \eqref{eq:monad_mu}. In the case
the bind operator \mint{haskell}{>>=} can be implemented as follows
\begin{minted}{haskell}
(>>=)  :: m a -> (a -> m b) -> m b
ma >>= f = join ( fmap f ma )
\end{minted} 
...
M<A> join(M< M<A> >);
\end{minted}
where \mint{c++}{pure} can be treated as $\eta$
\eqref{eq:monad_eta} and 
\mint{haskell}{join} as $\mu$ \eqref{eq:monad_mu}. In the case
...
def unit[A](x: A): M[A]
\end{minted} 
I.e. \mint{scala}{flatMap} can be considered as $\mu$ and
\mint{scala}{unit} as $\eta$. 
\end{example}

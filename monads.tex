...
\begin{figure}
  ...
  \caption{Commutative diagram for $\mu \circ (\eta \times \idm{M})
    \circ \lambda = \mu \circ (\idm{M} \times \eta) \circ \rho =
    \idm{M}$ .} 
...
\mu \circ (\idm{M} \times \eta) \circ \rho = 
\idm{M}
\]
...
\begin{definition}[Tensor product]
  ...
  f: m \times n \to a
  \]
  exists only one morphism $\tilde{f}: m \otimes n \to a$ such that
  \[
  f = \tilde{f} \circ \phi
  \]
...
\[
\mu \circ (\eta \star \idnt{M}) = \mu \circ (\idnt{M} \star \eta) =
\idnt{M}
\]
